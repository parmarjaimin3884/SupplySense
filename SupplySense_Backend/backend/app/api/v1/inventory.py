...
@router.get(
    ...
    description="Returns filtered and sorted inventory items across distribution hubs.",
)
async def list_inventory(
    ...
    try:
        stmt = select(Inventory, Product, Warehouse, Category).join(Product, Inventory.product_id == Product.id).join(Warehouse, Inventory.warehouse_id == Warehouse.id).outerjoin(Category, Product.category_id == Category.id)

        if search:
            q_term = f"%{search}%"
            stmt = stmt.where(or_(Product.name.ilike(q_term), Product.sku.ilike(q_term), Category.name.ilike(q_term)))
...
@router.get(
    ...
    description="Returns items where available quantity is at or below reorder level.",
)
async def get_low_stock(db: AsyncSession = Depends(get_db)) -> BaseResponse[List[InventoryItemResponse]]:
    ...
    stmt = select(Inventory, Product, Warehouse).join(Product, Inventory.product_id == Product.id).join(Warehouse, Inventory.warehouse_id == Warehouse.id).where(Inventory.available_quantity <= Product.reorder_level).limit(50)
    results = (await db.execute(stmt)).all()
...
@router.get(
    ...
    description="Returns items with zero available quantity.",
)
async def get_out_of_stock(db: AsyncSession = Depends(get_db)) -> BaseResponse[List[InventoryItemResponse]]:
    """Returns out of stock inventory items."""
    stmt = select(Inventory, Product, Warehouse).join(Product, Inventory.product_id == Product.id).join(Warehouse, Inventory.warehouse_id == Warehouse.id).where(Inventory.available_quantity == 0).limit(50)
    results = (await db.execute(stmt)).all()
...
@router.get(
    ...
    description="Returns non-moving or overstocked items.",
)
async def get_dead_stock(db: AsyncSession = Depends(get_db)) -> BaseResponse[List[InventoryItemResponse]]:
    """Returns dead stock / non-moving inventory."""
    stmt = select(Inventory, Product, Warehouse).join(Product, Inventory.product_id == Product.id).join(Warehouse, Inventory.warehouse_id == Warehouse.id).where(Inventory.available_quantity > 2000).limit(20)
    results = (await db.execute(stmt)).all()
...
@router.get(
    ...
    description="Returns stock movements, transfers, and inbound/outbound transactions.",
)
async def get_movements() -> BaseResponse[List[InventoryMovementResponse]]:
    """Returns historical inventory movement ledger."""
    movements = [
        InventoryMovementResponse(id="mov-001", warehouse_id="wh-1", warehouse_name="Surat Central", product_id="prod-1", product_name="MacBook Pro M4 16-inch", movement_type="INBOUND", quantity=150, reference_id="PO-2026-881", movement_date=date.today()),
        InventoryMovementResponse(id="mov-002", warehouse_id="wh-1", warehouse_name="Surat Central", product_id="prod-2", product_name="Dell XPS 15 Oled", movement_type="TRANSFER", quantity=40, reference_id="TR-2026-042", movement_date=date.today()),
    ]
...
async def get_inventory_by_id(id: str, db: AsyncSession = Depends(get_db)) -> BaseResponse[InventoryDetailResponse]:
    ...
    stmt = select(Inventory, Product, Warehouse).join(Product, Inventory.product_id == Product.id).join(Warehouse, Inventory.warehouse_id == Warehouse.id).where(Inventory.id == id)
    ...
    detail = InventoryDetailResponse(
        ...
        available_quantity=inv.available_quantity,
        damaged_quantity=inv.damaged_quantity,
        stock_status="OPTIMAL" if inv.available_quantity > prod.reorder_level else "LOW_STOCK",
        total_value=Decimal(str(inv.available_quantity)) * prod.cost_price,
        last_updated=inv.last_updated,
