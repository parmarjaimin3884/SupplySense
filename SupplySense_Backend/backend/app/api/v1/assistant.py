"""
SupplySense — AI Assistant API v1 Router
========================================
...
@router.post(
    ...
    description="Executes a natural-language manager query through the LangGraph Supervisor.",
)
async def chat(
    payload: AIChatRequest = Body(...),
    supervisor: SupplySenseSupervisor = Depends(get_supervisor),
) -> AIChatResponse:
    """Executes natural language manager query through LangGraph Supervisor."""
    start_t = time.time()
    try:
