"""
SupplySense - Executive Summary Agent Chains
Provides the structuring chain that converts free-text executive summary
...
def get_structuring_chain(llm: BaseChatModel):
    ...
    data matching the ExecutiveSummaryResponse Pydantic schema.
    Uses LangChain's `with_structured_output` for guaranteed schema compliance.
    """
    structured_llm = llm.with_structured_output(ExecutiveSummaryResponse)
