...
export function Card({ className, children, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn(
        'bg-white rounded-xl border border-slate-200/80 shadow-card hover:border-slate-300 transition-all duration-200',
        className
      )}
      {...props}
    >
...
export function CardHeader({ className, children, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div className={cn('p-5 pb-3 flex items-center justify-between border-b border-slate-100', className)} {...props}>
      {children}
    </div>
  );
}
