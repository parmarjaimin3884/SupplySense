...
async def merge_agent_outputs(
    ...
    """
    Merges outputs from all executed agents into a single unified SupervisorResponse.
    ...
    # Handle case where only 1 agent was executed
    if len(agent_outputs) == 1:
        agent_name = list(agent_outputs.keys())[0]
        out_data = agent_outputs[agent_name]
        return _build_single_agent_supervisor_response(
            user_question, intent, selected_agents, agent_name, out_data, nodes_executed, duration_ms
        )

    # Multi-agent or Hybrid case: Use LLM for intelligent output synthesis
    try:
        llm = get_llm()
        ...
        return _heuristic_fallback_merge(
